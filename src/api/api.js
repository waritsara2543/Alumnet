...
export async function getFeedById(
  ...
  let res = await api.get(
    `/student/feed/${major_id}/${faculty_id}/${campus_id}/${graduate_year}`
  );
  return res.data.results;
}
export async function getClassdirectoryById(
  major_id,
  faculty_id,
  campus_id,
  graduate_year
  ...
  let res = await api.get(
    `/student/classdirectory/${major_id}/${faculty_id}/${campus_id}/${graduate_year}`
  );
  return res.data.results;
}
...
export async function createworkplacebefore(
  workplace_name,
  position,
  student_id,
  start_work,
...
export async function getEventBypublicid(faculty_id, public_relation_id) {
  try {
    const res = await api.get(
      `/admin/geteventbyid/${faculty_id}/${public_relation_id}`
    );
    return res.data.results;
  } catch (err) {
  }
...
export async function getDatainGoogleSheets() {
  let res = await api.get(
    `https://sheets.googleapis.com/v4/spreadsheets/1L0p43e9RCRS_0sjcz7xz35Uoi5Ev2PxklGUGHeLPSlA/values/sheet1!A2:L300/?key=AIzaSyCXRNRo1tvekH4YLwrw2kKnqaHlpAwz7TA`
  );
...
export async function updateAddressLocation(tumbon,amphone, province,postcode,country,student_id) {
  var data = {
    tumbon: tumbon,
    amphone: amphone,
    province: province,
...
export async function getLocationByStudentid(student_id) {
  try {
    const res = await api.get(
      `/student/addresslocationbystudentid/${student_id}`
    );
    return res.data.results;
  } catch (err) {
  }
